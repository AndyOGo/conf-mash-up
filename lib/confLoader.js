...
function load(options, done) {
    ...
    if(argv.config) {
        globPattern = argv.config;

        glob(globPattern, globDone);

        function globDone(err, files) {
            if(err) {
                log(chalk.red('Error: Can\'t find ' + chalk.magenta(argv.config) + '! Please make sure that your path or glob is correct.'));
                log(err);

                log(chalk.yellow('glob: ') + chalk.magenta(globPattern));
            ...
            if((!files || !files.length) && globPattern !== cwd + '/' + argv.config) {
                globPattern = cwd + '/' + argv.config;
                return glob(globPattern, globDone);
            }
...
function initConfig(options, configs, done) {
    ...
    if(!options || !('init' in options) || typeof options.init !== 'object') {
        if(hasDone) {
...
function loadDefaultConfig(options, config, done) {
    ...
    if(!config || forceMerge) {
        ...
        log(chalk.green('Load default config from: ' + chalk.magenta( file )));
    }
